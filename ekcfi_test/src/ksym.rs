...
pub fn parse_ksym(line: &str) -> Result<Ksym, &'static str> {
    ...
    let addr_str = iter.next().ok_or("invalid syntex in kallsyms")?;
    let ty_str = iter.next().ok_or("invalid syntex in kallsyms")?;
    let name = iter.next().ok_or("invalid syntex in kallsyms")?;
    let addr =
        u64::from_str_radix(addr_str, 16).map_err(|_| "invalid address syntex in kallsyms")?;
    let ty = ty_str
        .chars()
        .nth(0)
        .map_or(Err("invalid symbol syntex in kallsyms"), |c| {
            <KsymType as TryFrom<char>>::try_from(c)
        })?;
    Ok(Ksym::new(addr, ty, name))
}

pub fn parse_kallsyms() -> Result<Vec<Ksym>, &'static str> {
    let mut result = Vec::new();

    for line in read_to_string("/proc/kallsyms")
        .map_err(|_| "failed to read kallsyms")?
        .lines()
    {
